...
def collapse_source(data,vel,theta):
	...
	for w in range(exp):
		for k in range(nz):
			diff[w,k,:,:]=shift(diff[w,k,:,:], (vely*(k+1),velx*(k+1)), order=3, mode='constant', cval=math.nan, prefilter=True)
	...
	col_source_final=np.array(toy[:,:,:])
	hdu=fits.PrimaryHDU(col_source_final)
	hdul=fits.HDUList([hdu])
	hdul.writeto("/home/stella/point_sources/sim1/det_images/col_source_final.fits")
