...
mx=65 # number of pixels in toy model's frame along x axis
my=65 # number of pixels in toy model's frame along y axis
px0=(mx-nx)/2 # x coordinate (using zero indexing) of POI's first pixel within toy model frame  ..-1=19
py0=(my-ny)/2 # y coordinate (using zero indexing) of POI's first pixel within toy model frame  19

px0=int(round(px0))
py0=int(round(py0))

if px0+nx+nz>mx or py0+ny+nz>my: 
...
for k in range(1,nz): 
     ...
     for n in range (nz): 
        for m in range(ny): 
           for l in range(nx): 
              toy[k,py0+m,px0+l+k]=toy[k,py0+m,px0+l+k]+diff[l,m,n]          

mv_source1=np.array(toy[:,:,:])
hdu=fits.PrimaryHDU(mv_source1)
hdul=fits.HDUList([hdu])
hdul.writeto("mv_source1.fits")
